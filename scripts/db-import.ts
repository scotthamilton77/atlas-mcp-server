...
const isValidBackupDir = (backupDir: string): boolean => {
  ...
  if (!resolvedBackupDir.startsWith(projectRoot + path.sep)) {
    logger.error(
      `Invalid backup directory: Path is outside the project boundary: ${resolvedBackupDir}`,
    );
    return false;
  }
  ...
  if (
    ...
    logger.error(
      `Invalid backup directory: Path does not exist or is not a directory: ${resolvedBackupDir}`,
    );
    return false;
  }
  ...
  for (const file of expectedFiles) {
    ...
    if (!resolvedFilePath.startsWith(resolvedBackupDir + path.sep)) {
      logger.error(
        `Invalid file path detected: ${resolvedFilePath} is outside the backup directory ${resolvedBackupDir}`,
      );
      // Decide how to handle: warning or failure. Let's warn for now.
      logger.warning(
        `Skipping check for potentially unsafe file path: ${resolvedFilePath}`,
      );
    ...
    if (!existsSync(resolvedFilePath)) {
      ...
      logger.warning(
        `Expected backup file not found: ${resolvedFilePath}. Import might be incomplete.`,
      );
    }
  }
  return true;
...
const runManualImport = async () => {
  ...
  if (!isValidBackupDir(resolvedBackupDir)) {
    // Pass the already resolved path
    process.exit(1);
  }
  ...
  try {
    ...
    const errorToLog =
      error instanceof Error
        ? error
        : new Error(
            error === null ? "Encountered a null error" : JSON.stringify(error),
          );
    logger.error("Manual database import failed:", errorToLog, {
