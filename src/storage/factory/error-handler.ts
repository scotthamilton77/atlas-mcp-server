...
export function createFactoryError(
  ...
  return createError(code, message, `StorageFactory.${operation}`, userMessage, metadata);
}
...
export class StorageFactoryErrorHandler {
   ...
   */
  handleCreateError(error: unknown, operation: string, context?: Record<string, unknown>): never {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    const errorDetails =
      error instanceof Error
        ? {
