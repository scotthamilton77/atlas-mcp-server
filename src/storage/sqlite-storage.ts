...
export class SqliteStorage implements TaskStorage {
      ...
      await this.connectionManager.executeWithRetry(async () => {
        try {
          ...
          return this.withDb(async db => {
            ...
            try {
              await Promise.all([fs.access(walPath), fs.access(shmPath)]);
        ...
        error:
          error instanceof Error
            ? {
                ...
                customProps: Object.getOwnPropertyNames(error).reduce(
                  (acc, key) => {
  ...
  async createTask(input: CreateTaskInput): Promise<Task> {
    if (!input.name) {
      throw createError(ErrorCodes.INVALID_INPUT, 'Task name is required', 'createTask');
    }
  ...
  async updateTask(path: string, updates: UpdateTaskInput): Promise<Task> {
    ...
    if (!existingTask) {
      throw createError(ErrorCodes.TASK_NOT_FOUND, 'Task not found', 'updateTask', path);
    }
  ...
  private async withDb<T>(operation: (db: Database) => Promise<T>, context?: string): Promise<T> {
    if (!this.db) {
      throw createError(ErrorCodes.STORAGE_ERROR, 'Database not initialized', 'withDb');
    }
  ...
  async getTask(path: string): Promise<Task | null> {
    ...
    return this.withDb(async db => {
      try {
        ...
        throw createError(ErrorCodes.STORAGE_READ, 'Failed to get task', errorMessage);
      }
    });
  }
  ...
  async getTasks(paths: string[]): Promise<Task[]> {
    return this.withDb(async db => {
      try {
        ...
        throw createError(ErrorCodes.STORAGE_READ, 'Failed to get tasks', errorMessage);
      }
    });
  }
  ...
  async getTasksByPattern(pattern: string): Promise<Task[]> {
    return this.withDb(async db => {
      try {
        ...
        throw createError(ErrorCodes.STORAGE_READ, 'Failed to get tasks by pattern', errorMessage);
      }
    });
  }
  ...
  async getTasksByStatus(status: TaskStatus): Promise<Task[]> {
    return this.withDb(async db => {
      try {
        ...
        throw createError(ErrorCodes.STORAGE_READ, 'Failed to get tasks by status', errorMessage);
      }
    });
  }
  ...
  async getSubtasks(parentPath: string): Promise<Task[]> {
    return this.withDb(async db => {
      try {
        ...
        const needsUpdate = tasks.some(
          task => task.parentPath === parentPath && !parent.subtasks.includes(task.path)
        );

        if (needsUpdate) {
          parent.subtasks = Array.from(
            new Set([
              ...parent.subtasks,
        ...
        throw createError(ErrorCodes.STORAGE_READ, 'Failed to get subtasks', errorMessage);
      }
    });
  }
  ...
  async deleteTasks(paths: string[]): Promise<void> {
    await this.inTransaction(async () => {
      return this.withDb(async db => {
        ...
        const tasksToDelete = await Promise.all(allPaths.map(path => this.getTask(path)));
        const validTasksToDelete = tasksToDelete.filter((t): t is Task => t !== null);

        // Find all parent paths that need updating
        const parentsToUpdate = new Set(
          validTasksToDelete.filter(t => t.parentPath).map(t => t.parentPath as string)
        );
        ...
        if (allPaths.length > 0) {
          ...
          await db.run(`DELETE FROM tasks WHERE path IN (${deletePlaceholders})`, ...allPaths);
        }
  ...
  async vacuum(): Promise<void> {
    return this.withDb(async db => {
      try {
        ...
        throw createError(ErrorCodes.STORAGE_ERROR, 'Failed to vacuum database', errorMessage);
      }
    });
  }
  ...
  async analyze(): Promise<void> {
    return this.withDb(async db => {
      try {
        ...
        throw createError(ErrorCodes.STORAGE_ERROR, 'Failed to analyze database', errorMessage);
      }
    });
  }
  ...
  async checkpoint(): Promise<void> {
    return this.withDb(async db => {
      try {
        ...
        throw createError(ErrorCodes.STORAGE_ERROR, 'Failed to checkpoint database', errorMessage);
      }
    });
  }
  ...
  > {
    return this.withDb(async db => {
      try {
        ...
        const byStatus = (statusStats as { status: string; count: number }[]).reduce(
        ...
        throw createError(ErrorCodes.STORAGE_ERROR, 'Failed to get storage metrics', errorMessage);
      }
    });
  }
  ...
  async clearAllTasks(): Promise<void> {
    return this.withDb(async db => {
      try {
        ...
        throw createError(ErrorCodes.STORAGE_ERROR, 'Failed to clear tasks', errorMessage);
      }
    });
  }
      ...
      return this.withDb(async db => {
        ...
        try {
          ...
          for (const task of orphanedTasks) {
            ...
            if (!dryRun) {
              await db.run('UPDATE tasks SET parent_path = NULL WHERE path = ?', task.path);
              fixCount++;
            }
          }
            ...
            if (subtaskRefs.size > 0) {
              ...
              for (const subtask of subtasks.map(r => this.rowToTask(r))) {
                if (subtask.parentPath !== task.path) {
                  issues.push(
                    `Task ${task.path} lists ${subtask.path} as subtask but parent_path mismatch`
                  );
                  if (!dryRun) {
  ...
  async beginTransaction(): Promise<void> {
    ...
    return this.withDb(async db => {
      try {
        ...
        throw createError(ErrorCodes.STORAGE_ERROR, 'Failed to begin transaction', errorMessage);
      }
    });
  }
  ...
  async commitTransaction(): Promise<void> {
    ...
    return this.withDb(async db => {
      try {
        ...
        throw createError(ErrorCodes.STORAGE_ERROR, 'Failed to commit transaction', errorMessage);
      }
    });
  }
  ...
  async rollbackTransaction(): Promise<void> {
    ...
    return this.withDb(async db => {
      try {
        ...
        throw createError(ErrorCodes.STORAGE_ERROR, 'Failed to rollback transaction', errorMessage);
      }
    });
  }
  ...
  async close(): Promise<void> {
    try {
      ...
      throw createError(ErrorCodes.STORAGE_ERROR, 'Failed to close database', errorMessage);
    }
  }
}
