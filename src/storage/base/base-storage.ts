...
export abstract class BaseStorage implements TaskStorage {
  ...
  constructor(protected readonly config: Required<StorageConfig>) {
  ...
  async updateTask(path: string, updates: UpdateTaskInput): Promise<Task> {
    ...
    return this.executeInTransaction(async () => {
      ...
      const updatedTask: Task = {
        ...
        parentPath:
          updates.parentPath === null ? undefined : (updates.parentPath ?? existingTask.parentPath),
  ...
  protected ensureInitialized(): void {
    if (!this.isInitialized) {
      throw createError(ErrorCodes.STORAGE_ERROR, 'Storage not initialized', 'ensureInitialized');
    }
    if (this.isClosed) {
      throw createError(ErrorCodes.STORAGE_ERROR, 'Storage is closed', 'ensureInitialized');
    }
  }
  ...
  protected async checkMaintenance(): Promise<void> {
    ...
    const checkpointInterval = (this.config.performance?.checkpointInterval ??
      DEFAULT_CONFIG.performance.checkpointInterval) as number;
    const vacuumInterval = (this.config.performance?.vacuumInterval ??
      DEFAULT_CONFIG.performance.vacuumInterval) as number;
