...
async function main(): Promise<void> {
  try {
    ...
    const validLogLevel = Object.values(LogLevels)
      .map(l => l.toLowerCase())
      .includes(logLevel || '')
                  ...
                  inputSchema: {
                    ...
                    properties: {
                      path: {
                        type: 'string',
                        description:
                          'Required: Unique path that identifies the task in the hierarchy. Use format "system/component/action" for automated workflows or "project/feature/task" for development work. Examples:\n- "system/services/api/deploy"\n- "project/auth/implement-oauth"',
                      },
                      name: {
                        type: 'string',
                        description:
                          'Required: Clear, descriptive name for the task. Should indicate the specific action or goal. Examples:\n- "Deploy API Service v2"\n- "Implement OAuth Authentication"',
                      },
                      description: {
                        type: 'string',
                        description:
                          'Optional but recommended: Detailed technical explanation including:\n- Success criteria\n- Implementation steps\n- Technical details\n- Rollback procedures\n- Error handling approach',
                      },
                      type: {
                        type: 'string',
                        enum: ['TASK', 'MILESTONE'],
                      ...
                      dependencies: {
                        ...
                        items: {
                          type: 'string',
                          description:
                            'Task path that must complete before this task can start. Use valid existing paths',
                        },
                        description:
                          'Optional: Tasks that must complete first. Order from upstream to downstream. Examples:\n- Service dependencies: ["system/database/migrate", "system/cache/initialize"]\n- Feature dependencies: ["project/auth/core", "project/api/endpoints"]',
                      },
                      metadata: {
                        type: 'object',
                        description:
                  ...
                  inputSchema: {
                    ...
                    properties: {
                      path: {
                        type: 'string',
                        description:
                          'Required: Path of the task to update. Must be an existing task path like "system/services/api" or "project/auth/oauth"',
                      },
                      updates: {
                        type: 'object',
                        description:
                          'Required: Changes to apply to the task. Include only fields that need updating. Each update should have clear reasoning',
                        properties: {
                          name: {
                            type: 'string',
                            description:
                              'Optional: New display name. Update when:\n- Clarifying task purpose\n- Reflecting changed requirements\n- Improving task identification',
                          },
                          description: {
                            type: 'string',
                            description:
                          ...
                          reasoning: {
                            type: 'string',
                            description:
                              'Optional but recommended: Clear explanation of why this task is needed and its intended purpose. This helps maintain context and aids in decision making.',
                          },
                          status: {
                            type: 'string',
                            enum: ['PENDING', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'BLOCKED'],
                            description:
                              'Optional: New task state with strict transition rules:\n\n' +
                              'Status Flow:\n' +
                          ...
                          dependencies: {
                            ...
                            items: {
                              type: 'string',
                              description:
                                'Task path that must complete first. Use valid existing paths',
                            },
                            description:
                              'Optional: New dependency list. Common updates:\n- Adding new requirements\n- Removing completed dependencies\n- Reordering execution sequence\n- Fixing broken dependencies',
                          },
                          metadata: {
                            type: 'object',
                            description:
              ...
              try {
                ...
                switch (name) {
                  ...
                  case 'bulk_task_operations':
                    result = await taskManager.bulkTaskOperations({
                      operations: args.operations as Array<{
                  ...
                  case 'repair_relationships':
                    result = await taskManager.repairRelationships(
                ...
                const errorMessage =
                  error instanceof BaseError ? error.getUserMessage() : String(error);
          ...
          try {
            ...
            if (taskManager) {
              await Promise.race([
                taskManager.cleanup(),
                new Promise((_, reject) =>
                  setTimeout(() => reject(new Error('Task manager cleanup timeout')), 10000)
                ),
              ]);
            }
            ...
            if (storage) {
              await Promise.race([
                storage.close(),
                new Promise((_, reject) =>
                  setTimeout(() => reject(new Error('Storage close timeout')), 5000)
                ),
              ]);
            }
