...
export class TaskValidators {
  ...
  async validateDependencyConstraints(
    ...
    const validTask = this.ensureTaskArrays(task);
    return this.dependencyValidator.validateDependencyConstraints(
      validTask,
      dependencies,
      getTaskByPath,
      mode
   ...
   */
  async sortTasksByDependencyGraph(graph: Map<string, Set<string>>): Promise<string[]> {
   ...
   */
  async validateTypeChange(task: Partial<Task>, newType: TaskType): Promise<void> {
    const validTask = this.ensureTaskArrays(task);
