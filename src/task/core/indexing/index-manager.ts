...
export class TaskIndexManager {
  ...
  async indexTask(task: Task): Promise<void> {
    try {
      ...
      throw createError(ErrorCodes.OPERATION_FAILED, 'Failed to index task', errorMessage);
    }
  }
  ...
  async getTasksByPattern(
    ...
    const matchingPaths = Array.from(this.taskIndexes.keys()).filter(path => regex.test(path));
  ...
  async unindexTask(task: Task): Promise<void> {
    try {
      ...
      throw createError(ErrorCodes.OPERATION_FAILED, 'Failed to unindex task', errorMessage);
    }
  }
  ...
  private cleanupPatterns(): void {
    ...
    for (const [pattern, paths] of this.patternIndex.entries()) {
      const validPaths = Array.from(paths).filter(
        path => this.getTaskFromWeakRef(path) !== undefined
      );
  ...
  private cleanupIndexes(): void {
    ...
    for (const [parent, children] of this.parentIndex.entries()) {
      const validPaths = Array.from(children).filter(
        path => this.getTaskFromWeakRef(path) !== undefined
      );
    ...
    for (const [dep, dependents] of this.dependencyIndex.entries()) {
      const validPaths = Array.from(dependents).filter(
        path => this.getTaskFromWeakRef(path) !== undefined
      );
