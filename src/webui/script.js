document.addEventListener('DOMContentLoaded', () => {
    ...
    const dom = {
        ...
        knowledgeViewModeToggle: document.getElementById('knowledge-view-mode-toggle'),
    };
    ...
    const utils = {
        ...
        parseJsonSafe: (jsonString, defaultValue = []) => {
            if (typeof jsonString !== 'string') return Array.isArray(jsonString) ? jsonString : defaultValue;
            try {
    ...
    const ui = {
        applyTheme: (theme) => {
            ...
            if (typeof mermaid !== 'undefined') {
                mermaid.initialize({
                    startOnLoad: false,
                    theme: theme === 'dark' ? config.MERMAID_THEME_DARK : config.MERMAID_THEME_LIGHT,
        ...
        toggleTheme: () => {
            ...
            if (state.showingTaskFlow) { // Re-render task flow if visible
                renderService.taskFlow(state.currentTasks, dom.taskFlowContainer);
            }
        },
        loadTheme: () => {
            const savedTheme = localStorage.getItem('atlasTheme') || config.DEFAULT_THEME;
    ...
    const apiService = {
        connect: async () => {
            ...
            try {
                ...
                state.driver = neo4j.driver(config.NEO4J_URI, neo4j.auth.basic(config.NEO4J_USER, config.NEO4J_PASSWORD));
                ...
                ui.showError(`Neo4j Connection Error: ${error.message}. Check console and credentials.`, true);
                dom.projectSelect.innerHTML = '<option value="">Neo4j Connection Error</option>';
                return false;
            }
        },
            ...
            try {
                ...
                return result.records.map(record => {
                    ...
                    record.keys.forEach(key => {
                        ...
                        if (neo4j.isInt(value)) {
                        ...
                        } else if (value && typeof value === 'object' && value.properties) { // Node
                            const nodeProps = {};
                            ...
                            obj[key] = nodeProps;
                        } else if (Array.isArray(value) && value.every(item => item && typeof item === 'object' && item.properties)) { // Array of Nodes
        ...
        fetchProjects: async () => {
            ...
            try {
                const projectsData = await apiService.runQuery('MATCH (p:Project) RETURN p.id as id, p.name as name ORDER BY p.name');
        ...
        fetchProjectDetails: async (projectId) => {
            ...
            ui.updateToggleButton(dom.taskFlowToggle, false, 'View Task List', 'View Task Flow');
            ui.clearError();

            try {
                const projectResult = await apiService.runQuery('MATCH (p:Project {id: $projectId}) RETURN p', { projectId });
                state.currentProject = projectResult.length > 0 ? projectResult[0].p : null;
                ...
                const tasksQuery = `
                    ...
                    ORDER BY task.title
                `;
                const tasksResult = await apiService.runQuery(tasksQuery, { projectId });
                state.currentTasks = tasksResult.map(r => ({ ...r.task, dependencyIds: r.dependencyIds || [] }));
                renderService.tasks(state.currentTasks, dom.tasksContent, state.tasksViewMode);
                ...
                renderService.knowledgeItems(state.currentKnowledgeItems, dom.knowledgeContent, state.knowledgeViewMode);
    ...
    const renderService = {
        projectDetails: (project, element) => {
            ...
            const urlsHtml = urlsToRender.length > 0
            ...
            if (project.dependencies && Array.isArray(project.dependencies) && project.dependencies.length > 0) {
                dependenciesText = project.dependencies.map(dep => utils.escapeHtml(dep)).join(', ');
        ...
        tasks: (tasks, element, viewMode) => {
            ...
            element.innerHTML = tasks.map(task => {
                ...
                try {
                    ...
                    const urlsHtml = taskUrlsToRender.length > 0
                    ...
                    const tagsHtml = task.tags && Array.isArray(task.tags) && task.tags.length > 0 ? `Tags: ${task.tags.map(t => utils.escapeHtml(t)).join(', ')}<br>` : '';
        ...
        knowledgeItems: (items, element, viewMode) => {
            ...
            element.innerHTML = items.map(item => {
                ...
                // Detailed view
                try {
                    const tagsHtml = item.tags && Array.isArray(item.tags) && item.tags.length > 0 ? `Tags: ${item.tags.map(t => utils.escapeHtml(t)).join(', ')}<br>` : '';
                    const citationsHtml = item.citations && Array.isArray(item.citations) && item.citations.length > 0 ? `Citations: <ul>${item.citations.map(c => `<li>${utils.escapeHtml(c)}</li>`).join('')}</ul>` : '';
        ...
        taskFlow: async (tasks, element) => {
            ...
            tasks.forEach(task => {
                ...
                const taskTitle = utils.escapeHtml(task.title || 'Untitled Task').replace(/"/g, '#quot;');
                ...
                if (task.dependencyIds && task.dependencyIds.length > 0) {
                    task.dependencyIds.forEach(depId => {
                        const dependencyId = (depId || 'unknown_dependency').replace(/"/g, '#quot;');
            ...
            try {
                ...
                const currentThemeSetting = document.documentElement.classList.contains('dark-mode') ? config.MERMAID_THEME_DARK : config.MERMAID_THEME_LIGHT;
                 mermaid.initialize({
                    startOnLoad: false,
    ...
    const eventHandlers = {
        ...
        handleTaskViewModeToggle: () => {
            state.tasksViewMode = state.tasksViewMode === 'detailed' ? 'compact' : 'detailed';
            ui.updateToggleButton(dom.taskViewModeToggle, state.tasksViewMode === 'compact', 'Detailed View', 'Compact View');
            renderService.tasks(state.currentTasks, dom.tasksContent, state.tasksViewMode);
        },
        handleKnowledgeViewModeToggle: () => {
            state.knowledgeViewMode = state.knowledgeViewMode === 'detailed' ? 'compact' : 'detailed';
            ui.updateToggleButton(dom.knowledgeViewModeToggle, state.knowledgeViewMode === 'compact', 'Detailed View', 'Compact View');
            renderService.knowledgeItems(state.currentKnowledgeItems, dom.knowledgeContent, state.knowledgeViewMode);
        },
        handleTaskFlowToggle: () => {
            ...
            ui.updateToggleButton(dom.taskFlowToggle, state.showingTaskFlow, 'View Task List', 'View Task Flow');
            if (state.showingTaskFlow) {
                renderService.taskFlow(state.currentTasks, dom.taskFlowContainer);
            }
        },
        setup: () => {
            dom.projectSelect.addEventListener('change', eventHandlers.handleProjectSelectChange);
            dom.refreshButton.addEventListener('click', eventHandlers.handleRefreshClick);
            dom.themeCheckbox.addEventListener('change', eventHandlers.handleThemeToggleChange);
            dom.themeLabel.addEventListener('click', () => dom.themeCheckbox.click()); // Allow clicking label
            dom.taskViewModeToggle.addEventListener('click', eventHandlers.handleTaskViewModeToggle);
            dom.knowledgeViewModeToggle.addEventListener('click', eventHandlers.handleKnowledgeViewModeToggle);
            dom.taskFlowToggle.addEventListener('click', eventHandlers.handleTaskFlowToggle);
        }
    };
